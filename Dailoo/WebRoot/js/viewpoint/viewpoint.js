...
*/
    ...
    $.get("../footer.html",function(data){
        ...
        $("#footer").html(data);
        setMapUrls($("#backward").attr('href').split('theme/')[1]);
        ...
        if (jinfeng.indexOf(vpData.theme[0].id) != -1) { //如果是金峰鄉主題
            ...
            changeCss("jinfeng");
        } else if(anping.indexOf(vpData.theme[0].id) != -1){ //如果是安平區主題
            changeCss("anping");
        } else if(taitongcounty.indexOf(vpData.theme[0].id) != -1){ //如果是台東縣主題
            changeCss("taitongcounty");
        } else if(guanshan.indexOf(vpData.theme[0].id) != -1){ //如果是關山鎮主題
            changeCss("guanshan");
        }*/
    });
}
